...
\begin{proof}
  ...
  $g$). By Deligne's congruence, $r_{\phi,k} = 0 \bmod g$.  Since $g$
  divides $x^{q^k}-x$, the recurrence
  relation~\eqref{eisenstein_recurrence} implies $r_{\phi,k+1} = 0
  \bmod g$. Since this recurrence has order~2, this further yields
  ...
  this implies $\bar h_{\phi,f} = 0 \bmod g$, since $g$ divides $f$.

  Conversely, assume $\bar h_{\phi,f} = 0 \bmod g$. To prove that
  $\phi$ has supersingular reduction at $g$, using Deligne's
  congruence, it suffices to show $r_{\phi,k} \bmod g=0$.  Since $\bar
  h_{\phi,f} = 0 \bmod g$ and $g$ divides $f$, we have $r_{\phi,n}
  \bmod g = 0$ and $r_{\phi,n+1} \bmod g=0$. In particular, if $g=f$,
...
$m-k \geq k$. By~\cite[Lemma~2.3]{cor},
\[
r_{\phi,m} = r_{\phi,k} r_{\phi,m-k}^{q^k} \bmod g,\quad r_{\phi,m+1} = r_{\phi,k} r_{\phi,m+1-k}^{q^k} \bmod g,
\]
