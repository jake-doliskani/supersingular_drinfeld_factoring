...
% Necessarily, $K$ contains $\F_q$ as a subfield. Fix an
% algebraic closure $\bar{K}$ of $K$ and let $\tau: \bar{K}
% \longrightarrow \bar{K}$ denote the $q^{th}$ power Frobenius
% endomorphism. 
...
% To every $\F_q[x]$-algebra $L$ over $\bar{K}$, the Drinfeld module $\phi$
% endows a new $\F_q[x]$-module structure (which, we denote by $\phi(L)$)
% through the $\F_q[x]$-action $$\forall f \in L, \forall a \in \F_q[x], a*f =
% \phi_a(f).$$ For every $\F_q[x]$-algebra homomorphism $\rho:L
% \longrightarrow L$, $\forall a\in \F_q[x]$ and $\forall f \in L$,
% $\rho(\phi(f)) = \phi(\rho(f))$. Thus $\rho$, when thought of as a
% map from $\phi(L) \longrightarrow \phi(L)$, is an $\F_q[x]$-module
% homomorphism. For every direct product $L \times L^\prime$ of
% $\F_q[x]$-algebras over $\bar{K}$, we hence have the corresponding direct
% sum of $\F_q[x]$-modules $$\phi(L \times L^\prime) \cong \phi(L) \oplus
% \phi(L^\prime).$$

% Henceforth, we restrict our attention to Drinfeld modules $\phi:\F_q[x]
% \longrightarrow \F_q(x)\langle\tau\rangle$ over $\F_q(x)$ (with
% $\gamma : \F_q[x] \rightarrow \F_q(x)$ being the inclusion (identity),
% $g_\phi(x)\in \F_q[x]$ and $\Delta_\phi(x)\in \F_q[x]^\times$) and their
% reductions.
