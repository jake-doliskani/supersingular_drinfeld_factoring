...
In this article, we give an algorithm the computation of the Hasse
invariante of elliptic modules over finite fields, and show how fast
...
coefficients, using the fact that the coefficients of the
...
Strassen~\cite{Strassen76} and the Chudnovsky and
...
$\tau^k$ in $\phi(f)\bmod f =\sum_{i=0}^{2k} h_i \tau^i$, with $h_i$
...
algorithms, namely {\em modular composition}, turns out to be faster
in the boolean model; we discuss this further in the next section).
...
matrix multiplication; we can always take $\omega \le 2.37$
\cite{CoWi90,LeGall14}. Then, our first main result is the following
theorem.

\begin{theorem}\label{theo:1}
  Let $\phi: \F_q[x] \to \F_q(x)\langle \tau \rangle$ be an elliptic
...
algorithm, and runtime reported here, are the same as that in that
...
we believe it is worth stating such results, since it brings a new
...
We take a different approach. To factor a squarefree polynomial $g$,
...
prime ideals $\langle f \rangle$ in $\F_q[x]$, $\phi$ has
supersingular reduction; this is the case if and only if $h_{\phi,f}$
vanishes. The properties of the Hasse invariant $h_{\phi,g}$ computed
modulo $g$ then imply that for any prime factor $f$ of $g$,
$h_{\phi,g} = 0 \bmod f$ if and only if $f$ is a prime with
supersingular reduction for $\phi$. 
...
$n^{3/2+\varepsilon} (\log q)^{1 +o(1)}
+ n^{1+\varepsilon}(\log q)^{2+o(1)}$ bit operations.

As mentioned above, these results do not improve on the
state-of-the-art for polynomial factorization. Since Berlekamp's
randomized polynomial time algorithm~\cite{ber}, polynomial
factorization over finite fields has been the subject of a vast body
of work. Among important milestones, we mention~\cite{cz,gs}; the
first subquadratic algorithms are due to Kaltofen and Shoup~\cite{ks},
with runtimes $O(n^{1.815}\log q)$ operations in $\F_q$.
...
q)^{1+o(1)}$ in a boolean model, and exhibited the corresponding
...
that algorithm with runtime $n^{3/2+\varepsilon} (\log q)^{1 +o(1)} +
...
details for their computation, proving Theorem~\ref{theo:1}.  In
...
some prime power $q$, and let $L$ be a field equipped with a
...
invariants of $\phi$ at primes of degree $k$. Defining this sequence
allows us to define a generalization of $h_{\phi,f}$ to
...
The following subsection gives our algorithm to compute $h_{\phi, f}$
for an arbitrary $f$. We summarize here the main results we will need;
...
and still outperfom implementations of the Kedlaya-Umans algorithm. As
...
is well-defined (as $A(\xi_i)$, for an arbitrary lift of $a$ to
$\F_q[x]$) and satisfies $a(\xi_i)=\tau^i(a)$.  In particular, for any
$i,j$ in $\Z$, the relation $\xi_i(\xi_j)=\xi_{i+j}$ holds.

The following items describe technical results needed in our main
...
\item[{\bf 4.}] Given $(a_0,\dots,a_{k-1})$ in $\K$ and $\xi_i$, for
  ...
  $\F_q$, by exploiting the fact that the second argument is the same
  for all instances.
...
\item[{\bf 5.}] Given integers $u,\ell,m \in O(n)$, and $\xi_1$, one
  ...
  compute $\xi_{-u}$ for $O(\log u)$ modular compositions, and finally
...
\item [{\bf 6.}] For $u,\ell,m$ as above, given $(a_0,\dots,a_{m-1})$
  ...
  this is what we will need below. As above, in the boolean model, the
  cost is easily seen to be $m n^{1+\varepsilon} (\log q)^{1+o(1)}$
  bit operations.
...
elliptic module $\phi$ over $\F_q(x)$, we devise an efficient
...
in that algorithm.  Another inspiration for this algorithm is
...
(\xi^{q^{k-1}}-x)\delta^{q^{k-2}} \rho_{k-2},
\end{equation}
...
\[A_{i + j} = \tau^{i + j}(\mathcal{A})(\xi) = \tau^i\Big(\tau^j(\mathcal{A})\big( 
\tau^{-i}(\xi)\big)\Big)\]
to deduce that  for all $i \ge 1$,
\[\tau^{i}\Big(\mathcal{B} \big( \tau^{-i}(\xi)\big) \Big) = A_{i + \ell-1} \cdots A_{i + 1}A_{i} = B_{i,i+\ell}.\]
In particular, for any integer $m$, $B_{0,m\ell}$ can be computed as the product of the  matrices
\[
\tau^{(m-1) \ell}\Big(\mathcal{B} \big( \tau^{-(m-1) \ell}(\xi)\big) \Big), \dots, 
\tau^{\ell}\Big(\mathcal{B} \big( \tau^{-\ell}(\xi)\big) \Big), \mathcal{B} \big(\xi\big);
\]
more generally, for any integers $m,u$, we have 
\[
B_{u,u+m\ell}=
\tau^{u+(m-1) \ell}\Big(\mathcal{B} \big( \tau^{-(u+(m-1) \ell)}(\xi)\big) \Big) \cdots
\tau^{u+\ell}\Big(\mathcal{B} \big( \tau^{-(u+\ell)}(\xi)\big) \Big)
\tau^{u}\Big(\mathcal{B} \big( \tau^{-u}(\xi)\big) \Big).
\]
...
\rfloor$. Then, $v=u+m\ell$ satisfies $v \le n-1$ and $n-v-1 \le
(n-1-u)^\beta$.  The discussion above suggests Algorithm
\ref{alg:hasse-inv} for computing $B_{u,v}$.

\begin{algorithm}[H]
  \caption{Main subroutine for the Hasse invariant}
  \label{alg:hasse-inv}
  \begin{algorithmic}[1]
    \REQUIRE $f \in \F_q[x]$,  $\delta, \gamma \in \K=\F_q[x]/f$, integers $u,n$, with $u < n-1$
...
algebraic and boolean model, where the main difference will lie in the
...
\item A first solution for Step \ref{step:hasse-2} is to compute $
  ...
  to the one in the previous item. In the boolean model, since $\xi^q$
...
(n-1)^\beta$), and so on. The cost of the first call dominates the
overall runtime, so that Theorem~\ref{theo:1} is proved.
...
This suggests that we could use a Drineld module $\phi$ in a
...
\begin{remark}
  In Step $4$, we may compute $r_{\phi,k} \bmod f$ for some $k\geq n$
...
removed using a root finding algorithm; it takes $O(\MM(n)\log n
...
\{d\}$.Hence, we might have run into issues of bad reduction if $f$
...
As mentioned in~\eqref{equ:fact-sep}, $\gcd(r_{\phi,n} \bmod f, f)$ is
the product of all irreducible factors of $f$ that are supersingular
with respect to $\phi$. Thus, our algorithm separates the irreducible
factors supported at the supersingular primes from those supported at
the ordinary primes. In the following, we show that for an elliptic
module chosen randomly as in Step \ref{step:fac-3}, the splitting of
$f$ in Step \ref{step:fac-split} is random enough to ensure that the
...
In the following lemma, we establish our claim on the probability of
...
\begin{lemma}
  ...
  Suppose that $f$ is not irreducible and that $12(n+2) \le
...
\begin{proof}
  ...
  \item[(b)] exactly one of $\langle f_1\rangle, \langle f_2\rangle$
    splits in $\F_q(x)(\sqrt{x-a})$ if and only if $x-a$ splits in
    either $K_1$ and $K_2$, or none of them.
  \end{enumerate}
  ...
  for any $\tau=(\tau_1,\tau_2) \in \Z/2\Z \oplus \Z/2\Z$, 
  we obtain that the number $N_\tau$ of degree 1 primes in $\F_q(x)$ that are unramified in $K_1K_2$ 
  and of symbol $\tau$ satisfies
  \begin{align*}
  \left|N_\tau - \frac{q}{4} \right| & \le  \frac 12 \left ( (4+g(K_1K_2)) q^{1/2} + 4 q^{1/4}  + (4 + g(K_1K_2)) \right)  
   %% &\le \frac 32 (4+g(K_1 K_2)) q^{1/2},
  \end{align*}
  where $g(K_1K_2)$ is the genus of $K_1K_2$. Taking into account the
  prime at infinity, we obtain that the number $N_a$ of degree one
  primes $\{\langle x-a\rangle, a \in \F_q\}$ that are neither totally
  inert nor totally split in $K_1K_2$, and the number $N_b$ of degree
  one primes $\{\langle x-a\rangle, a \in \F_q\}$ that are either
  totally inert or totally split in $K_1K_2$ satisfy
  $$ N_a \ge N_{(0,1)} + N_{(1,0)} -1, \quad N_b \ge N_{(0,0)} + N_{(1,1)} -1.$$
  The previous inequalities imply
  \begin{align*}
N_a, N_b & \ge \frac 12 q - \left ( (4+g(K_1K_2)) q^{1/2} + 4 q^{1/4}  + (5 + g(K_1K_2)) \right)  \\
&\ge \frac 12 q - 3(4+g(K_1K_2)) q^{1/2}.
  ...
  $$N_a, N_b \ge \frac 12 q - 3(n+2) q^{1/2},$$ 
...
computing the $7$-th Hasse invariant lift $r_{\phi, 7} \bmod f$, we
find $a = \gcd(r_{\phi, 10}, f) = 1 + 4x + x^2 + 4x^3 + x^4$; this is
the product of the factors of $f$ that are supuersingular with respect
to $\phi$. Now, $a$ does not pass the irreducibility check, so the
process is repeated for $a$. The next randomly generated supersingular
elliptic module is
\[
\begin{array}{rll}
	d & = & 5 + x, \\
...
\end{array}
\]
The Hasse invariant lift of this $\phi$ splits $a$ as $
\tilde a = \gcd(r_{\phi, 4}, a) = 4 + 6x + x^2$, and $a/\tilde a = 2
+ 5x + x^2$ which are both irreducible; again, this means that
$\tilde a$ is supuersingular and $a/\tilde a$ is ordinary with respect to
$\tilde \phi$. Finally, $f / a = 2 + 5x + 6x^2 + 3x^3 + 6x^4 + 6x^5 + x^6$
